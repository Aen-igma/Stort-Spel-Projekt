		...
		if(m_pMesh)  {
			...
			for(uint32_t i = 0; i < m_pMesh->m_partitions.size(); i++)  {
				...
				if(pMaterial) {
					...
					RenderSystem::BindShaderResourceView<PShader>(8u, renderer.m_shadowMap);
					...
					RenderSystem::BindSamplers<PShader>(3u, renderer.m_borderSampler);
					...
					RenderSystem::UnBindShaderResources<PShader>(8u, 1u);
